...
import { DrinksMenuModal } from "../packages/DrinksMenuModal";
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  // Navbar only hides for gallery lightbox
  const isHidden = isGalleryOpen;
  ...
  return (
    <>
          ...
          {mobileMenuOpen && (
            <div className="md:hidden mt-3 p-5 sm:p-6 rounded-3xl bg-white/95 backdrop-blur-md border border-[#3A5A40]/20 shadow-2xl flex flex-col gap-3 sm:gap-4 text-center max-h-[85vh] overflow-y-auto">
              ...
              <button
                onClick={() => {
                  setMobileMenuOpen(false);
                  setIsDrinksMenuOpen(true);
                }}
                className="font-bold text-base sm:text-lg text-[#2C3E2E] hover:text-[#3A5A40] py-2 border-b border-[#3A5A40]/10"
              >
                Karta Pića
              </button>
              <a
      ...
      <DrinksMenuModal
        isOpen={isDrinksMenuOpen}
        onClose={() => setIsDrinksMenuOpen(false)}
      />
    </>
  );
};
